...
class PortScanner:
    def __init__(self, detectOS=False, detectServices=False, scanTcp=True) -> None:
        ...
        self.os_ports = [21, 22, 23, 25, 53, 80, 443]
    ...
    def guessOS(self, target):
        for port in self.os_ports:
            for _ in range(self.max_retries):
                ...
                if result == errno.EAGAIN:
                    ...
                    banner = self.getPortServiceBanner(target, port).lower()
    ...
    def scanUdpPort(self, target, port):
        for _ in range(self.max_retries):
            try:
        ...
        if port == self.DNS_PORT:
            return port, "closed", "DNS (?)"
        elif port == self.SNMP_PORT:
            return port, "closed", "SNMP (?)"
        else:
            return port, "closed", "Uknown"

    def scanUdpPorts(self, target):
        results: List[PortStatus] = []
        try:
            with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
                futures = {
                    executor.submit(self.scanUdpPort, target, port): port
                    for port in self.ports
                }
                for future in concurrent.futures.as_completed(futures):
                    port, status, banner = future.result()
                    results.append({"port": port, "status": status, "service": banner})
                return results
    ...
    def scanTcpPort(self, target, port):
        for _ in range(self.max_retries):
            ...
            if result == errno.EAGAIN:
                ...
                if self.detectServices:
                    banner = self.getPortServiceBanner(target, port)
                else:
                    banner = "Unknown"
                return port, "open", banner
            elif result in self.filtered_error_codes:
...
def main():
    ...
    if args.sU:
        args.p = "53,161"  # ignore port input when using udp scanning
    ...
    if args.sU:
        args.O = False  # disable OS detection during UDP

    print(yellow("OS Detection: ") + bold(str(args.O)))

    if args.sU:
        args.sV = False  # disable service grabbing during UDP
    ...
    if args.sU == False:  # TCP
        for address in scanner.addresses:
            ...
            print(dim(f"Not shown: {len(results)-len(filtered_results)} closed ports"))

            if scanner.detectOS:
                os = scanner.guessOS(address)
                print(yellow("\nOperating System: ") + os)
        ...
        for address in scanner.addresses:
            ...
            results = scanner.scanUdpPorts(address)
            results = [
                {**item, "port": f"{item['port']}/udp"}
                for item in results
                if "port" in item
            ]
            header = [field.capitalize() for field in results[0].keys()]
