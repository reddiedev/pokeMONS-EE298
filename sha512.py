...
def sha512(orig_message_bytes):
    ...
    for chunk_element in chunks:
        ...
        for i in range(16, 80):
            s0 = (right_rotate(array[i-15], 1) ^ right_rotate(array[i-15], 8) ^ (array[i-15] >> 7))
            s1 = (right_rotate(array[i-2], 19) ^ right_rotate(array[i-2], 61) ^ (array[i-2] >> 6))
        ...
        for i in range(80):
            ...
            ch = (e & f) ^ ((~e) & g)
            temp1 = (h + S1 + ch + constants[i] + array[i]) & 0xFFFFFFFFFFFFFFFF
